			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.covers && response.covers.length > 0) {
						...
						for (var i = 0; i < response.covers.length; i += 2) { // Each slick item contains 2 covers vertically
							sliderHtml += '<div class="item">'; // Start a new slick-slide item
							// Process up to 2 covers in each slider item (for vertical stacking)
							for (var j = 0; j < 2; j++) {
								if (i + j < response.covers.length) {
									var cover = response.covers[i + j];
									var mockupSrc = '/storage/' + cover.mockup; // cover.mockup is relative path
									var overlaySrc = cover.random_template_overlay_url; // This is a full URL
									
									var marginClass = (j < 1 && i + j + 1 < response.covers.length) ? 'mb-3' : ''; // Margin for the first of two vertical items
						...
						if (typeof $.fn.slick === 'function') {
							$targetPane.find(".slick_slider_tab").slick({
								infinite: true,
								slidesToShow: 3, // Display 3 items (columns)
								slidesToScroll: 1,
								responsive: [
									{breakpoint: 1199, settings: {slidesToShow: 2}},
									{breakpoint: 767, settings: {slidesToShow: 1}},
