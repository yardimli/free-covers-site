...
AppAdmin.Items = (function() {
	...
	function loadItems(itemType, page = 1, searchQuery = '', coverTypeIdFilter = '') {
		...
		let ajaxData = {
			type: itemType, // 'action' key is not used by backend for listItems
			page: page,
				...
				if (response.success) {
					...
					if (items.length === 0) {
						...
						items.forEach(item => {
							...
							rowHtml += ` <button class="btn btn-info btn-sm me-1 generate-ai-metadata" data-id="${item.id}" data-type="${itemType}" title="Generate AI Metadata"> <i class="fas fa-wand-magic-sparkles"></i> </button>
                                         <button class="btn btn-warning btn-sm me-1 edit-item" data-id="${item.id}" data-type="${itemType}" title="Edit"> <i class="fas fa-edit"></i> </button>
                                         <button class="btn btn-danger btn-sm delete-item" data-id="${item.id}" data-type="${itemType}" title="Delete"> <i class="fas fa-trash-alt"></i> </button>
                                     </td>`;
							rowHtml += `</tr>`;
